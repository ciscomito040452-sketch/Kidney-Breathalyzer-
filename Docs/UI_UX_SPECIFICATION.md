...
CSS-only animations via `lib/motion/` + classes ใน `app/globals.css` — **ไม่ใช้** framer-motion

### Tokens

| Token | Value | ใช้เมื่อ |
|-------|-------|----------|
| `--motion-duration-fast` | 200ms | backdrop, dismiss, press |
| `--motion-duration-normal` | 320ms | page enter, stagger, sheet |
| `--motion-duration-slow` | 450ms | ring fill, chart, count-up |
| `--motion-ease-enter` | cubic-bezier(0, 0, 0.2, 1) | เข้าหน้าจอ |
| `--motion-ease-exit` | cubic-bezier(0.4, 0, 1, 1) | ออกจากหน้าจอ |
| `--motion-ease-standard` | cubic-bezier(0.25, 0.1, 0.25, 1) | ทั่วไป |

### Components / Classes

| Class / Component | ใช้เมื่อ |
|-------------------|----------|
| `MotionPage` / `kb-page-enter` | tab routes — fade + slide up |
| `MotionPushPage` / `kb-page-push` | drill-down (result, guide) |
| `MotionCrossfade` | onboarding step, filter change |
| `Pressable` / `kb-pressable` | tap scale 0.97 |
| `StaggerSection` / `kb-fade-up` | section โผล่ทีละชั้น |
| `SheetScaffold` | bottom sheet + backdrop |
| `kb-nav-indicator` | bottom nav sliding pill |

### ห้ามทำ

- Typing animation บน AI
- Confetti / parallax / bounce เกินจำเป็น
- ไม่เคารพ `prefers-reduced-motion`
